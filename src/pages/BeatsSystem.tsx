...
// import { fetchData, updateStatus } from '../services/BeatsService';
...
//             const  updatedLocations = sampleData.map(async (loc) => {
...
const BeatsSystem = () => {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          const updatedLocations = sampleData.map(async (loc) => {
            ...
            console.log("Distance : ", distance, " , ID : ", loc._id);
          ...
          // Optional: Await updates and set new sample data if needed
          // Promise.all(updatedLocations).then(setSampleData);
        },
        (error) => {
          console.error('Error getting location:', error);
        }
  ...
  const getFeatureByRegionId = (data: any, ps: any) => {
    const feature = data.features.find((f: any) => f.properties.ps === ps);
    return {
      type: "FeatureCollection",
      features: [feature],
  ...
  const getStyle = (feature: any) => {
    ...
    return {
      fillColor: colors[feature.properties.regionId] || 'gray',
      weight: 3,
      opacity: 1,
      fillOpacity: 0.6
    };
  ...
  const currentLocationIcon = new L.Icon({
    iconUrl: 'https://cdn-icons-png.flaticon.com/512/684/684908.png', // or use your own asset
    iconSize: [32, 32],
    iconAnchor: [16, 32],
    popupAnchor: [0, -32]
  });
  ...
  const getInitialPosition = () => {
    ...
    return {
      center: [14.6782, 77.6086], // Default to Ananthapuramu
      zoom: 12
    };
  };
    ...
    >
      ...
      {sampleData.map((value, index) => (
        <Marker key={index} position={[value.latitude, value.longitude]}>
          <Popup>
            <strong>{value.accusedName}</strong><br />
            <b>Phone:</b> {value.mobileNumber}<br />
